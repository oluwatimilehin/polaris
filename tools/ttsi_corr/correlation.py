...
def read_scores(filepath: Path, default_precision: str | None) -> ScoreDict:
    ...
    Returns:
        ScoreDict: Dictionary mapping (wlname, devname, wlgroup, wlinstance) tuples
                  to score dictionaries containing projection scores, architecture name, and metadata
...
def compare_scores(
    ...
    correlation ratios and organizing the data for output. Uses the actual architecture
    name from the simulation data. Conditionally includes columns based on data
    ...
    for key in sorted(common_keys):
        ...
        projected_ttft_ms = actual_scores[key].get('ttft_ms')
        projected_archname = actual_scores[key].get('archname', 'Unknown')
        ...
        row_data: dict[str, Any] = {
            'Workload': key[0],
            'Arch': projected_archname,  # Use actual architecture name from simulation data
